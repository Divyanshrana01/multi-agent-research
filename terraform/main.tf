# tells terraform which providers we need (aws + random) and where to
# keep the state file (s3 bucket, with dynamodb for locking so two people
# can't run apply at the same time)
terraform {
  required_providers {
    aws = {
...
# which aws region to actually create resources in
provider "aws" {
  region = var.aws_region
}

# ─── Variables ───────────────────────────────────────────────────────────────
# these are all the inputs you can override when running terraform apply
# (e.g. -var="app_cpu=1024"). they all have defaults so it works out of the box
...
# ─── Data & Locals ────────────────────────────────────────────────────────────
# look up which availability zones exist in this region, then just grab 2 of them
data "aws_availability_zones" "available" {}

locals {
  azs          = slice(data.aws_availability_zones.available.names, 0, 2)
  # if we were given an ssl cert, turn https on, otherwise stay http-only
  https_enabled = var.acm_certificate_arn != ""
}

# ─── VPC ──────────────────────────────────────────────────────────────────────
# the network everything lives in: 2 public subnets (internet facing, for the
# load balancer) and 2 private subnets (backend stuff like db/redis/ecs tasks)
...
# lets stuff in the private subnets reach these aws services directly,
# without needing a nat gateway (nat gateways cost money, this is cheaper)
...
# ─── Security Groups ──────────────────────────────────────────────────────────
# firewall rules - who's allowed to talk to what, on which ports
...
# ─── Bedrock Guardrail ────────────────────────────────────────────────────────
# aws bedrock's built-in content safety filter - blocks hate/violence/sexual/
# self-harm content, blocks certain prompt attacks, and masks PII like ssn/emails
...
# ─── ElastiCache Redis ────────────────────────────────────────────────────────
# managed redis cache, sits in the private subnets, used for semantic cache +
# session memory + job queue
...
# ─── RDS PostgreSQL ───────────────────────────────────────────────────────────
# managed postgres db, this is where long-term memory / app data lives
...
# ─── ALB ──────────────────────────────────────────────────────────────────────
# load balancer sitting in front of everything - routes traffic to the main
# app (port 8000) and the pyrit red-team dashboard (port 8001)
...
# ─── ECS ──────────────────────────────────────────────────────────────────────
# the container cluster + the iam roles it needs:
# execution role = permission to pull images / read secrets to start the task
# task role = permission the app itself needs while running (call bedrock etc)
...
# ─── Secrets Manager ─────────────────────────────────────────────────────────
# one big json blob with all our config/secrets in it - app/config.py reads
# this at startup instead of us hardcoding secrets anywhere
...
# blueprint for each container: what image to run, what ports it needs,
# where its logs go. "app" task also runs tensorzero as a sidecar container
...
# ─── ECS Services ─────────────────────────────────────────────────────────────
# actually runs the task definitions above and keeps them alive + wired to
...
# ─── ECS Auto-Scaling ─────────────────────────────────────────────────────────
# adds/removes app tasks automatically based on cpu usage, between
# app_min_capacity and app_max_capacity
...
# ─── ECR ──────────────────────────────────────────────────────────────────────
# private docker registries - where we push the app/pyrit/tensorzero images
# that the ecs task definitions above reference
...
# cron-like rule that fires every monday 2am and kicks off the pyrit
# task to run an automated red-team security test against our app
...
# ─── Outputs ──────────────────────────────────────────────────────────────────
# stuff terraform prints after "apply" finishes, so we know the urls/endpoints
# to actually use (load balancer url, ecr repo urls, db/redis endpoints)
