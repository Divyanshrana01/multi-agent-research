# Each agent below calls out to TensorZero (self-hosted LLM gateway) for inference,
...
async def _tz_call(config: Config, function_name: str, message: str) -> str:
    # wrapped in a lambda so with_retry gets a fresh call each attempt,
    # not a single already-awaited coroutine
...
async def _tz_call_once(config: Config, function_name: str, message: str) -> str:
    # function_name selects which TensorZero function config to run (defines
    # model, prompt template, etc.) — response shape matches OpenAI's content blocks
...
class CriticAgent:
    ...
    async def run(self, report: str) -> bool:
        ...
        # truncate so a long report doesn't blow the critic's context window
...
def build_graph(config: Config):
    ...
    # verify branches based on orchestrator.route()'s return value: "search"
    # loops the pipeline back, END finishes the graph
